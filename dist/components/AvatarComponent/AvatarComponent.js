...
function AvatarGroup({ max = 5, size = "md", className, children }) {
    ...
    return (_jsxs("div", { className: `${styles.group} ${className || ""}`, children: [visible.map((child, i) => (_jsx("div", { className: styles.groupItem, style: { zIndex: visible.length - i }, children: child }, i))), overflow > 0 && (_jsx("div", { className: `${styles.avatar} ${styles[size]} ${styles.overflow}`, children: _jsxs("span", { className: styles.initials, children: ["+", overflow] }) }))] }));
}
